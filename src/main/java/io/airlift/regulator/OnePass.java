...
final class OnePass
{
            ...
            Workspace workspace)
    {
        ...
        if (captureSlotCount > MAX_CAP) {
            ...
            return searchExtendedCaptures(prog, context, textBegin, textEnd, matchKind, submatch, matchCount, captureSlotCount, workspace);
        }
        ...
        if (matchCount == 0) {
            return searchBoolean(prog, context, textBegin, textEnd, matchKind);
        }
        if (matchCount == 1) {
            return searchGroupZero(prog, context, start, end, matchKind, submatch);
        }
        ...
        if (useCallerCaptures) {
            captures = submatch;
            matchCaptures = submatch;
        }
        else if (workspace != null) {
            workspace.prepare(captureSlotCount);
            captures = workspace.captures;
            matchCaptures = workspace.matchCaptures;
        }
        else {
            captures = new int[captureSlotCount];
            matchCaptures = new int[captureSlotCount];
        }
        Arrays.fill(captures, 0, captureSlotCount, -1);
        if (captures != matchCaptures) {
            Arrays.fill(matchCaptures, 0, captureSlotCount, -1);
        }
    ...
    // PERFORMANCE-SENSITIVE HOT LOOP: boolean matching does not need capture state. Keep this
    // separate from the capture loop so performance does not depend on scalar replacement of
    // scratch arrays after the shared matcher has seen both capture and non-capture workloads.
    private static boolean searchBoolean(Prog prog, Slice context, int textBegin, int textEnd, Prog.MatchKind matchKind)
    {
        byte[] bytes = context.byteArray();
        byte[] bytemap = prog.bytemapArray();
        int range = prog.bytemapRange();
        ...
        for (; position < textEnd; position++) {
            ...
            int condition = stateAction[stateOffset + byteClass];
    ...
    static boolean searchGroupZero(Prog prog, Slice context, int start, int end, Prog.MatchKind matchKind, int[] submatch)
    {
        int matchEnd = searchGroupZeroEnd(prog, context, start, end, matchKind);
        if (matchEnd < 0) {
            submatch[0] = -1;
            submatch[1] = -1;
            return false;
        ...
        submatch[0] = 0;
        submatch[1] = matchEnd - context.byteArrayOffset() - start;
        return true;
    }

    // PERFORMANCE-SENSITIVE HOT LOOP: keep group-zero result materialization outside this method
    // so the generated loop carries only the scalar match end.
    static int searchGroupZeroEnd(Prog prog, Slice context, int start, int end, Prog.MatchKind matchKind)
    {
        byte[] bytes = context.byteArray();
        byte[] bytemap = prog.bytemapArray();
        int range = prog.bytemapRange();
        ...
        int position = context.byteArrayOffset() + start;
        int textEnd = context.byteArrayOffset() + end;
        int state = 0;
        int nextMatchCondition = stateMatchCondition[state];
        int stateOffset = 0;
        int matchEnd = -1;
    ...
    static final class Workspace
    {
        ...
        private void prepare(int captureSlotCount)
        {
            if (captures == null || captures.length < captureSlotCount) {
                int[] newCaptures = new int[captureSlotCount];
                int[] newMatchCaptures = new int[captureSlotCount];
                captures = newCaptures;
                matchCaptures = newMatchCaptures;
            }
        }
    }
