...
REGULATOR_DEPENDENCIES="${ROOT_DIR}/target/re2-memory-census-regulator-classpath.txt"
...
CLASSPATH="${ROOT_DIR}/target/test-classes:${ROOT_DIR}/target/classes:$(cat "${REGULATOR_DEPENDENCIES}"):${JONI_JAR}:${JOL_JAR}"
javac \
...
java \
    ...
    io.airlift.regulator.RegexpMemoryCensus \
    "${census_arguments[@]}"
