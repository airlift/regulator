...
def compare(regulator_rows, joni_rows):
    if set(regulator_rows) != set(joni_rows):
        missing_regulator = sorted(set(joni_rows) - set(regulator_rows))
        missing_joni = sorted(set(regulator_rows) - set(joni_rows))
        raise ValueError(f"benchmark key mismatch: missing Regulator={missing_regulator}, missing Joni={missing_joni}")

    rows = []
    for (operation, workload, source_length) in sorted(regulator_rows):
        regulator_result = regulator_rows[(operation, workload, source_length)]
        joni_result = joni_rows[(operation, workload, source_length)]
        rows.append({
            "operation": operation,
            "workload": workload,
            "source_length": source_length,
            "regulator_time_ns": regulator_result["time_ns"],
            "joni_time_ns": joni_result["time_ns"],
            "regulator_to_joni_time_ratio": regulator_result["time_ns"] / joni_result["time_ns"],
            "regulator_allocation_bytes": regulator_result["allocation_bytes"],
            "joni_allocation_bytes": joni_result["allocation_bytes"],
            "allocation_difference_bytes": regulator_result["allocation_bytes"] - joni_result["allocation_bytes"],
            **({
                "regulator_before_time_ns": regulator_result["before_time_ns"],
                "regulator_after_time_ns": regulator_result["after_time_ns"],
                "regulator_bracket_ratio": max(
                    regulator_result["before_time_ns"] / regulator_result["after_time_ns"],
                    regulator_result["after_time_ns"] / regulator_result["before_time_ns"]),
            } if "before_time_ns" in regulator_result else {}),
        })
    return rows


def combine_regulator_bracket(before_rows, after_rows):
    if set(before_rows) != set(after_rows):
        ...
        raise ValueError(f"Regulator bracket key mismatch: missing before={missing_before}, missing after={missing_after}")
...
def statistics_for(rows):
    ratios = [row["regulator_to_joni_time_ratio"] for row in rows]
    ...
    if "regulator_bracket_ratio" in rows[0]:
        result["maximum_regulator_bracket_ratio"] = max(row["regulator_bracket_ratio"] for row in rows)
    return result
...
def write_markdown(path, rows, summary):
    largest_time = sorted(rows, key=lambda row: row["regulator_to_joni_time_ratio"], reverse=True)[:12]
    ...
    with path.open("w") as output:
        ...
        output.write("Time ratios are Regulator divided by Joni; lower is better. Allocation is reported as absolute bytes per operation. ")
        if "regulator_bracket_ratio" in rows[0]:
            output.write("Regulator time is the geometric mean of the runs immediately before and after Joni.\n\n")
        else:
            output.write("\n\n")
        output.write("## Summary\n\n")
        bracketed = "regulator_bracket_ratio" in rows[0]
        bracket_header = " Maximum Regulator bracket |" if bracketed else ""
        ...
        for operation, values in [("overall", summary["overall"]), *summary["operations"].items()]:
            bracket_value = f" {values['maximum_regulator_bracket_ratio']:.3f}x |" if bracketed else ""
            output.write(
...
def write_rows(output, rows):
    output.write("| Operation | Workload | Source | Regulator time | Joni time | Time ratio | Regulator allocation | Joni allocation | Allocation difference |\n")
    output.write("|---|---|---:|---:|---:|---:|---:|---:|---:|\n")
    for row in rows:
        output.write(
            f"| {row['operation']} | {row['workload']} | {row['source_length']} | "
            f"{row['regulator_time_ns']:.1f} ns | {row['joni_time_ns']:.1f} ns | "
            f"{row['regulator_to_joni_time_ratio']:.3f}x | {row['regulator_allocation_bytes']:.0f} B | "
            f"{row['joni_allocation_bytes']:.0f} B | {row['allocation_difference_bytes']:.0f} B |\n")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("regulator_json", type=Path)
    parser.add_argument("joni_json", type=Path)
    parser.add_argument("output_directory", type=Path)
    parser.add_argument("--regulator-after", type=Path)
    arguments = parser.parse_args()

    arguments.output_directory.mkdir(parents=True, exist_ok=True)
    regulator_rows = read_jmh(arguments.regulator_json, "")
    if arguments.regulator_after:
        regulator_rows = combine_regulator_bracket(regulator_rows, read_jmh(arguments.regulator_after, ""))
    rows = compare(regulator_rows, read_jmh(arguments.joni_json, "Joni"))
