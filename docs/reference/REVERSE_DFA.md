...
A forward DFA finds where a match ends. When the caller also needs the start,
RE2 can search backward from that end with a reverse-compiled program. The
reverse DFA finds the original match start. Searches already using an NFA-style
engine for captures do not need this extra step.
...
the public matcher falls back to NFA, BitState, or OnePass as appropriate.
A DFA failure means "try another engine," not "no match."
...
- `TestDfaReversePosition` checks exact two-phase boundaries, assertions,
  Unicode, and empty matches.
