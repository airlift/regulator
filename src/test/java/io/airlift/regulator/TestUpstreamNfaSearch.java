...
public class TestUpstreamNfaSearch
{
    ...
    public void testLeftmostFirstVsLeftmostLongest()
    {
        ...
        int[] first = new int[4];
        assertThat(Nfa.search(prog, text, true, Prog.MatchKind.FIRST_MATCH, first)).isTrue();
        ...
        assertThat(Nfa.search(prog, text, true, Prog.MatchKind.LONGEST_MATCH, longest)).isTrue();
    ...
    @Test
    public void testUnanchoredLeftmostFirstMatch()
    {
        Prog prog = compile("ab|cd", Regexp.LIKE_PERL | Regexp.LATIN1);
        Slice text = latin1("xabcdx");

        int[] m = new int[2];
        assertThat(Nfa.search(prog, text, false, Prog.MatchKind.FIRST_MATCH, m)).isTrue();
        assertThat(m[0]).isEqualTo(1);
        assertThat(m[1]).isEqualTo(3);
        assertThat(sliceLatin1(text, m[0], m[1])).isEqualTo("ab");
    }
    ...
    @Test
    public void testBeginTextAnchorRequiresContextBegin()
    {
        Prog prog = compile("^a$", Regexp.LIKE_PERL | Regexp.LATIN1);

        Slice context = latin1("xa");

        assertThat(Nfa.search(prog, context, 1, 2, false, Prog.MatchKind.FIRST_MATCH, null)).isFalse();
        assertThat(Nfa.search(prog, latin1("a"), false, Prog.MatchKind.FIRST_MATCH, null)).isTrue();
        assertThat(Nfa.search(prog, latin1("xa"), false, Prog.MatchKind.FIRST_MATCH, null)).isFalse();
    }

    @Test
    public void testEndTextAnchorRequiresMatchEnd()
    {
        Prog prog = compile("a$", Regexp.LIKE_PERL | Regexp.LATIN1);

        assertThat(Nfa.search(prog, latin1("ba"), false, Prog.MatchKind.FIRST_MATCH, null)).isTrue();
        assertThat(Nfa.search(prog, latin1("ab"), false, Prog.MatchKind.FIRST_MATCH, null)).isFalse();
    }
    ...
    public void testAnyByteStarGreedyVsNonGreedy()
    {
        ...
        int[] g = new int[2];
        assertThat(Nfa.search(greedy, text, true, Prog.MatchKind.FIRST_MATCH, g)).isTrue();
        assertThat(g[0]).isEqualTo(0);
        assertThat(g[1]).isEqualTo(3);

        Prog nonGreedy = compile("\\C*?", Regexp.LIKE_PERL | Regexp.LATIN1);
        int[] ng = new int[2];
        assertThat(Nfa.search(nonGreedy, text, true, Prog.MatchKind.FIRST_MATCH, ng)).isTrue();
