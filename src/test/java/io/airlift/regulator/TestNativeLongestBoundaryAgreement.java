...
{
    private static final String RESOURCE = "io/airlift/regulator/testing/native_longest_boundary.jsonl";
    ...
    private static void assertNativeCase(GoldenJsonl.JsonObject testCase)
    {
        ...
        if (start == 0 && end == text.length()) {
            ...
            switch (anchor) {
                case UNANCHORED -> {
                    assertThat(regexp.find(text)).as("find").isEqualTo(expectedMatched);
                    assertResult(regexp.findResult(text), expectedMatched, expectedGroups);
                }
                case ANCHOR_BOTH -> {
                    assertThat(regexp.matches(text)).as("matches").isEqualTo(expectedMatched);
                    assertResult(regexp.matchesResult(text), expectedMatched, expectedGroups);
                }
                case ANCHOR_START -> {}
            }
        }
