...
def benchmark_name(class_name, method_name):
    return f"io.airlift.regulator.{class_name}.{method_name}"
...
def build_pairs():
    ...
    for case_name in ("Easy0", "Easy1", "Medium", "Hard", "Parens"):
        add_sized_pairs(
            ...
            "failed-search",
            case_name.lower(),
            (("dfa", "DfaBoolean", "DFA"), ("re2", "Re2", "RE2")),
            SEARCH_SIZES,
            "BenchmarkRe2Search",
            f"search{case_name}",
    ...
    for case_name, java_prefix, native_prefix, specialized_engine in (
        ...
        add_sized_pairs(
            ...
            "successful-search",
            case_name,
            (("dfa", "DfaBoolean", "DFA"), ("re2", "Re2FullMatch", "RE2")),
            SEARCH_SIZES,
            "BenchmarkRe2SearchExtra",
            java_prefix,
            native_prefix)
    ...
    for case_name, java_method, native_name in (
            ("dot", "dotMatchCaptures", "DotMatchRE2/threads:1"),
            ("ascii", "asciiMatchCaptures", "ASCIIMatchRE2/threads:1"),
            ("possible-trivial", "programPossibleMatchRangeTrivial", "PossibleMatchRange_Prog_Trivial"),
            ("possible-complex", "programPossibleMatchRangeComplex", "PossibleMatchRange_Prog_Complex"),
            ("possible-prefix", "programPossibleMatchRangePrefix", "PossibleMatchRange_Prog_Prefix"),
            ("possible-no-program", "programPossibleMatchRangeNoProg", "PossibleMatchRange_Prog_NoProg")):
    ...
    for case_name, java_method, native_prefix in (
            ("dot-star", "matchesDotStarLatin1", "FullMatch_DotStar_CachedRE2"),
            ("dot-star-dollar", "matchesDotStarDollarLatin1", "FullMatch_DotStarDollar_CachedRE2"),
            ("dot-star-capture", "matchesDotStarCaptureLatin1", "FullMatch_DotStarCapture_CachedRE2")):
...
def select_pairs(class_name=None):
    ...
    class_prefix = f"io.airlift.regulator.{class_name}."
...
def summarize(java_path, native_before_path, native_after_path, pairs=None, expected_unpaired_java=None):
    ...
    for pair in pairs:
        ...
        rows.append({
            ...
            "regulator_native_ratio": java_result["median"] / native_reference,
...
def write_outputs(rows, output_directory):
    ...
    for (family, engine), group in sorted(groups.items()):
        summaries.append({
            ...
            "engine": engine,
            "rows": len(group),
            "geometric_ratio": geometric_mean([row["regulator_native_ratio"] for row in group]),
            "median_ratio": statistics.median(row["regulator_native_ratio"] for row in group),
            "maximum_ratio": max(row["regulator_native_ratio"] for row in group),
    ...
    document = {
        "pair_count": len(rows),
        "all_row_geometric_ratio": geometric_mean([row["regulator_native_ratio"] for row in rows]),
...
def main():
    ...
    if arguments.command == "java-filter":
        ...
        if arguments.class_name:
            class_prefix = f"io.airlift.regulator.{arguments.class_name}."
