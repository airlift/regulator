...
public class TestTraditionalBenchmarkInputs
{
    ...
    @Test
    public void testMiscBenchmarksRequestNativeCaptureOutputs()
    {
        BenchmarkRe2Misc.MiscState state = new BenchmarkRe2Misc.MiscState();
        state.setup();
        BenchmarkRe2Misc benchmark = new BenchmarkRe2Misc();
        int[] expected = {0, state.httpText.length(), 0, state.httpText.length()};
        Arrays.fill(state.oneCaptureGroup, -1);
        assertThat(benchmark.dotMatchCaptures(state)).isSameAs(state.oneCaptureGroup).containsExactly(expected);
        Arrays.fill(state.oneCaptureGroup, -1);
        assertThat(benchmark.asciiMatchCaptures(state)).isSameAs(state.oneCaptureGroup).containsExactly(expected);
    }

    @Test
    public void testPhoneBenchmarkRequestsBothNativeCaptures()
    {
        BenchmarkRe2Parse.PhoneSearchState state = new BenchmarkRe2Parse.PhoneSearchState();
        state.textSize = 8;
        state.setup();
        BenchmarkRe2Parse benchmark = new BenchmarkRe2Parse();
        assertThat(benchmark.searchPhoneRe2Captures(state)).isSameAs(state.twoCaptureGroups).containsExactly(8, 22, 8, 14, 14, 22);
    }

    @Test
    public void testAnchoredRe2BenchmarksRequireFullMatch()
    {
        BenchmarkRe2SearchExtra.SearchState state = new BenchmarkRe2SearchExtra.SearchState();
        state.text = Slices.utf8Slice("ab");
        state.re2Success = Re2BenchmarkRunner.compileRe2("a");
        state.re2Success1 = state.re2Success;
        state.re2AltMatch = state.re2Success;
        BenchmarkRe2SearchExtra benchmark = new BenchmarkRe2SearchExtra();
        assertThat(benchmark.searchSuccessRe2FullMatch(state)).isFalse();
        assertThat(benchmark.searchSuccess1Re2FullMatch(state)).isFalse();
        assertThat(benchmark.searchAltMatchRe2FullMatch(state)).isFalse();
        state.text = Slices.utf8Slice("a");
        assertThat(benchmark.searchSuccessRe2FullMatch(state)).isTrue();
        assertThat(benchmark.searchSuccess1Re2FullMatch(state)).isTrue();
    ...
    public void testPossibleMatchRangeMatchesUpstreamMaximumLength()
    {
        ...
        BenchmarkRe2Misc benchmark = new BenchmarkRe2Misc();
        assertThat(benchmark.programPossibleMatchRangePrefix(state))
