...
class TestSummarize(unittest.TestCase):
    def testReducesCompleteMeasurements(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            rows.extend(self.measurements("curated/count-a", "count", "1000", {
                ...
                summarize.REGULATOR_ENGINE: "1.00us",
                summarize.REGULATOR_OBJECT_ENGINE: "1.25us",
            }))
            rows.extend(self.measurements("curated/count-b", "count", "1000", {
                ...
                summarize.REGULATOR_ENGINE: "1us",
                summarize.REGULATOR_OBJECT_ENGINE: "1us",
            }))
            rows.extend(self.measurements("curated/compile", "compile", "", {
                ...
                summarize.REGULATOR_ENGINE: "2.01ms",
                summarize.REGULATOR_OBJECT_ENGINE: "2ms",
            }))
            ...
            self.assertAlmostEqual(count_a["regulator_to_re2_bundled_ratio"], 1.0)
            self.assertAlmostEqual(count_a["regulator_to_regulator_object_ratio"], 0.8)
            self.assertAlmostEqual(count_a["native_before_after_drift"], 500 / 495 - 1)
            self.assertEqual(count_a["regulator_coefficient_of_variation"], 0)
    ...
    def testRequiresEveryNamedComparator(self):
        ...
        self.assertReductionFails(rows, "regulator/re2-object")

    def testReducesMeasurementsWithoutBundledEngine(self):
        rows = self.complete_measurements()
        rows = [row for row in rows if row["engine"] != summarize.BUNDLED_ENGINE]
        with tempfile.TemporaryDirectory() as temporary_directory:
            directory = Path(temporary_directory)
            input_path = directory / "raw.csv"
            self.write_measurements(input_path, rows)

            normalized, models, buckets = summarize.reduce_results(
                input_path,
                directory / "output",
                exclude_bundled_engine=True)

            self.assertEqual(normalized[0]["re2_bundled_median_ns"], "")
            self.assertEqual(normalized[0]["regulator_to_re2_bundled_ratio"], "")
            self.assertNotIn(summarize.BUNDLED_ENGINE, {row["comparator"] for row in models})
            self.assertNotIn(summarize.BUNDLED_ENGINE, {row["comparator"] for row in buckets})
    ...
    def testCanExcludeExcessiveNativeDrift(self):
        ...
        rows.extend(self.measurements("curated/unstable", "count", "100", {
            ...
            summarize.REGULATOR_ENGINE: "100ns",
            summarize.REGULATOR_OBJECT_ENGINE: "100ns",
        }))
        with tempfile.TemporaryDirectory() as temporary_directory:
            directory = Path(temporary_directory)
            input_path = directory / "raw.csv"
    ...
    def testRejectsWrongCampaignProvenance(self):
        rows, manifest_rows = self.campaign_measurements()
        object_row = next(row for row in rows if row["engine"] == summarize.REGULATOR_OBJECT_ENGINE)
        object_row["engine_version"] = "Regulator RE2 (native-access=true)"
        with tempfile.TemporaryDirectory() as temporary_directory:
            directory = Path(temporary_directory)
            input_path = directory / "raw.csv"
            manifest_path = directory / "manifest.csv"
            self.write_measurements(input_path, rows)
            self.write_manifest(manifest_path, manifest_rows)
            with self.assertRaisesRegex(ValueError, "object Regulator runner reported native access"):
                summarize.reduce_results(input_path, directory / "output", manifest_path)
    ...
    def campaign_measurements(self):
        ...
        versions = {
            ...
            summarize.REGULATOR_ENGINE: "Regulator RE2 (native-access=true)",
            summarize.REGULATOR_OBJECT_ENGINE: "Regulator RE2 (native-access=false)",
        }
