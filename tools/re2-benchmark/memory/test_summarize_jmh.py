...
class TestSummarizeJmh(unittest.TestCase):
    def testCombineRegulatorBracket(self):
        key = ("count", "literalSparse", 1024)
        rows = summarize_jmh.combine_regulator_bracket(
    ...
    def testRejectsMismatchedRegulatorBracket(self):
        with self.assertRaisesRegex(ValueError, "Regulator bracket key mismatch"):
            summarize_jmh.combine_regulator_bracket(
    ...
    def testComparison(self):
        ...
        self.assertEqual(rows[0]["regulator_to_joni_time_ratio"], 0.5)
