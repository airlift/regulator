...
public class BenchmarkRe2PublicApi
{
    ...
    {
        return data.pattern.lookingAt(data.source);
    }

    @Benchmark
    public boolean matches(BenchmarkData data)
    {
        return data.pattern.matches(data.source);
    }

    @Benchmark
    public boolean findInRange(BenchmarkData data)
    {
        return data.pattern.find(data.source, 1, data.source.length() - 1);
    }
    ...
    {
        return data.pattern.findResult(data.source);
    }

    @Benchmark
    public Slice replaceFirst(BenchmarkData data)
    {
        return data.pattern.replaceFirst(data.source, data.rewrite);
    }

    @Benchmark
    public Slice replaceAll(BenchmarkData data)
    {
        return data.pattern.replaceAll(data.source, data.rewrite).result();
    }

    @Benchmark
    public Slice extract(BenchmarkData data)
    {
        return data.pattern.extract(data.source, data.rewrite);
    }
