...
public class BenchmarkTrinoRegexpSearchEdges
{
    ...
    public static class BenchmarkData
    {
        ...
        public void setup()
        {
            ...
            switch (workload) {
                ...
                case "finalLineLiteralNoMatch" -> {
                    patternText = "a$";
                    Arrays.fill(sourceBytes, (byte) 'b');
                    finalLineFeed = true;
                }
                case "finalLineLiteralLateMatch" -> {
                    patternText = "a$";
                    Arrays.fill(sourceBytes, (byte) 'b');
                    match = new byte[] {'a'};
                    finalLineFeed = true;
                    matchStart = sourceBytes.length - match.length - 1;
                }
                case "finalLineCharacterClassNoMatch" -> {
                    patternText = "[a-z]+$";
                    Arrays.fill(sourceBytes, (byte) '1');
                    finalLineFeed = true;
                }
                case "finalLineCharacterClassLateMatch" -> {
                    patternText = "[a-z]+$";
                    Arrays.fill(sourceBytes, (byte) '1');
                    match = new byte[] {'a', 'b', 'c'};
                    finalLineFeed = true;
                    matchStart = sourceBytes.length - match.length - 1;
                }
                case "finalLineAlternationNoMatch" -> {
                    patternText = "(?:foo|bar)+$";
                    Arrays.fill(sourceBytes, (byte) '.');
                    finalLineFeed = true;
                }
                case "finalLineAlternationLateMatch" -> {
                    patternText = "(?:foo|bar)+$";
                    Arrays.fill(sourceBytes, (byte) '.');
    ...
    @Benchmark
    public boolean containsGeneral(BenchmarkData data)
    {
        return data.regexp.pattern().matchInto(data.source, UNANCHORED, null);
    }

    @Benchmark
    public boolean lookingAt(BenchmarkData data)
    {
        return data.regexp.pattern().lookingAt(data.source);
    }

    @Benchmark
    public boolean lookingAtGeneral(BenchmarkData data)
    {
        return data.regexp.pattern().matchInto(data.source, ANCHOR_START, null);
    }
