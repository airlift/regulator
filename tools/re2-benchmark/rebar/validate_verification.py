...
CORE_EXPECTED_ENGINES = {
    "re2/pinned-portable",
    "re2/pinned-host-tuned-before",
    "re2/pinned-host-tuned-after",
    "regulator/re2",
    "regulator/re2-object",
}
EXPECTED_ENGINES = {"re2", *CORE_EXPECTED_ENGINES}
EXPECTED_WORKLOAD_COUNT = 41


def read_manifest(path, expected_engines):
...
def validate(path, manifest_path=None, exclude_bundled_engine=False):
    expected_engines = CORE_EXPECTED_ENGINES if exclude_bundled_engine else EXPECTED_ENGINES
    workloads = defaultdict(dict)
    with path.open(newline="", encoding="utf-8") as input_file:
        for line_number, row in enumerate(csv.reader(input_file), start=1):
            if len(row) != 5:
    ...
    expected_workloads = None if manifest_path is None else read_manifest(manifest_path, expected_engines)
...
def main():
    ...
    try:
        ...
        validate(arguments.input.resolve(), manifest, arguments.exclude_bundled_engine)
