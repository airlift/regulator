...
final class CharClassBuilder
{
    ...
    private final ArrayList<RuneRange> ranges = new ArrayList<>();
    ...
    public void addCharClass(CharClass characterClass, int parseFlags)
    {
        ...
        if (ranges.isEmpty() && !excludeNewline && !foldCase) {
            // CharClass and RuneRange are immutable, so an unmodified class can reuse its canonical ranges.
            ranges.ensureCapacity(characterClass.rangeCount());
            for (int rangeIndex = 0; rangeIndex < characterClass.rangeCount(); rangeIndex++) {
                RuneRange runeRange = characterClass.range(rangeIndex);
                ranges.add(runeRange);
                if (runeRange.low() <= 'z' && runeRange.high() >= 'A') {
                    updateAsciiBitmaps(runeRange.low(), runeRange.high());
                }
            }
            runeCount = characterClass.runeCount();
            return;
        }
