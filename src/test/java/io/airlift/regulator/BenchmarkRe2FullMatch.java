...
import static io.airlift.regulator.Re2BenchmarkRunner.compileRe2Latin1;
...
public class BenchmarkRe2FullMatch
{
    ...
    @Benchmark
    public boolean matchesDotStar(FullMatchState state)
    {
        return state.re2DotStar.matches(state.text);
    }

    @Benchmark
    public boolean matchesDotStarDollar(FullMatchState state)
    {
        return state.re2DotStarDollar.matches(state.text);
    }

    @Benchmark
    public boolean matchesDotStarCapture(FullMatchState state)
    {
        return state.re2DotStarCapture.matches(state.text);
    }

    // Latin1 mode

    @Benchmark
    public boolean matchesDotStarLatin1(FullMatchState state)
    {
        return state.re2DotStarLatin1.matches(state.text);
    }

    @Benchmark
    public boolean matchesDotStarDollarLatin1(FullMatchState state)
    {
        return state.re2DotStarDollarLatin1.matches(state.text);
    }

    @Benchmark
    public boolean matchesDotStarCaptureLatin1(FullMatchState state)
    {
        return state.re2DotStarCaptureLatin1.matches(state.text);
    }
