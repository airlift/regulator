...
public final class Tester
{
    ...
    private static String formatMode(int flags)
    {
        ...
        return String.format(Locale.ROOT, "0x%08x", flags);
    }
    ...
    private final class TestInstance
    {
        ...
        private TestInstance(MatchKind kind, ParseMode mode)
        {
            ...
            try {
                ...
                this.error = true;
                this.errorMessage = "parse failed: regexpBase64=" + regexpBase64 + " mode=" + mode.desc() + " status=" + parseException.errorCode();
            ...
            this.capturingGroupCount = parsed.capturingGroupCount();
            this.prog = compiled;
            this.reverseProg = reverseCompiled;
            this.publicRegexp = config.includePublicApi() ? Re2.compile(Slices.wrappedBuffer(regexpBytes.byteArray(), regexpBytes.byteArrayOffset(), regexpBytes.length()), mode.parseFlags()) : null;
        }
        ...
        {
            int submatchCount = 1 + capturingGroupCount;
