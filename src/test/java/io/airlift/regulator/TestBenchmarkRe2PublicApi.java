...
public class TestBenchmarkRe2PublicApi
{
    ...
    public void testParameters()
    {
        ...
        for (String workload : new String[] {"tinyMatch", "tinyNoMatch", "largeMatch", "largeNoMatch"}) {
            ...
            assertThat(benchmark.find(data)).isEqualTo(expectedMatch);
            assertThat(benchmark.lookingAt(data)).isFalse();
            assertThat(benchmark.matches(data)).isFalse();
            assertThat(benchmark.findInRange(data)).isEqualTo(expectedMatch);
            assertThat(benchmark.matchIntoReusedBuffer(data)).isEqualTo(expectedMatch);
            assertThat(benchmark.findResult(data) != null).isEqualTo(expectedMatch);
            assertThat(benchmark.replaceFirst(data)).isNotNull();
            assertThat(benchmark.replaceAll(data)).isNotNull();
            assertThat(benchmark.extract(data) != null).isEqualTo(expectedMatch);
