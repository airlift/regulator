...
class TestSummarize(unittest.TestCase):
    def testCorrectedContractsCannotReuseOldMeasurements(self):
        pairs = SUMMARIZE.build_pairs()
        ranges = [pair for pair in pairs if '/possible-' in pair.identifier]
        self.assertEqual(4, len(ranges))
        self.assertTrue(all(pair.native_benchmark.startswith('PossibleMatchRange_Prog_') for pair in ranges))
        dfa = [pair for pair in pairs if pair.engine == 'dfa']
        self.assertEqual(87, len(dfa))
        self.assertTrue(all(pair.java_benchmark.endswith('DfaBoolean') for pair in dfa))
        captures = [pair for pair in pairs if pair.identifier in ('practical/dot/re2', 'practical/ascii/re2') or pair.identifier.startswith('successful-search/phone/')]
        self.assertEqual(10, len(captures))
        self.assertTrue(all(pair.java_benchmark.endswith('Captures') for pair in captures))
        anchored = [pair for pair in pairs if pair.engine == 're2' and pair.identifier.startswith(('successful-search/success/', 'successful-search/success-one-byte/', 'successful-search/alternate-match/'))]
        self.assertEqual(24, len(anchored))
        self.assertTrue(all(pair.java_benchmark.endswith('Re2FullMatch') for pair in anchored))
    ...
    def testPairManifestIsStableAndUnique(self):
        ...
        full_match_benchmarks = {
            pair.java_benchmark for pair in pairs if pair.identifier.startswith("full-match/")
        }
        self.assertEqual(full_match_benchmarks, {
            "io.airlift.regulator.BenchmarkRe2FullMatch.matchesDotStarCaptureLatin1",
            "io.airlift.regulator.BenchmarkRe2FullMatch.matchesDotStarDollarLatin1",
            "io.airlift.regulator.BenchmarkRe2FullMatch.matchesDotStarLatin1",
        })
    ...
    def testSummarizesCompleteResults(self):
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(2.1, rows[0]["regulator_native_ratio"])
    ...
    def testRejectsUnexpectedResult(self):
        ...
        java_rows.append({
            "benchmark": "io.airlift.regulator.Unexpected.benchmark",
            "primaryMetric": {
                "scoreUnit": "ns/op",
                "rawData": [[20]],
            },
