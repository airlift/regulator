...
REGULATOR_ENGINE = "regulator/re2"
REGULATOR_OBJECT_ENGINE = "regulator/re2-object"
...
CORE_REQUIRED_ENGINES = (
    PINNED_PORTABLE_ENGINE,
    PINNED_HOST_BEFORE_ENGINE,
    PINNED_HOST_AFTER_ENGINE,
    REGULATOR_ENGINE,
    REGULATOR_OBJECT_ENGINE,
)
REQUIRED_ENGINES = (BUNDLED_ENGINE, *CORE_REQUIRED_ENGINES)
CORE_COMPARATORS = (
    (PINNED_PORTABLE_ENGINE, "regulator_to_re2_pinned_portable_ratio"),
    (PINNED_HOST_ENGINE, "regulator_to_re2_pinned_host_tuned_ratio"),
)
COMPARATORS = ((BUNDLED_ENGINE, "regulator_to_re2_bundled_ratio"), *CORE_COMPARATORS)
RATIO_BUCKETS = (
    "faster-than-0.90x",
    "within-0.90x-1.10x",
    "1.10x-1.25x",
...
ROW_FIELDS = (
    ...
    "regulator_to_re2_bundled_ratio",
    "regulator_to_re2_pinned_portable_ratio",
    "regulator_to_re2_pinned_host_tuned_ratio",
    "regulator_object_to_re2_pinned_host_tuned_ratio",
    "regulator_to_regulator_object_ratio",
    "native_before_after_drift",
    "regulator_coefficient_of_variation",
...
def validate_campaign_provenance(workloads, manifest, selected_manifest=None):
    ...
    for name, measurements in workloads.items():
        ...
        if measurements[REGULATOR_ENGINE]["model"] != manifest[name]["model"]:
            raise ValueError(f"Workload {name!r} model differs from manifest")
        if (
                measurements[REGULATOR_ENGINE]["model"] != "compile" and
                measurements[REGULATOR_ENGINE]["haystack_len"] != manifest[name]["haystack_length"]):
            raise ValueError(f"Workload {name!r} haystack length differs from manifest")
        if selected_manifest is not None and measurements[REGULATOR_ENGINE]["model"] != selected_manifest[name]["model"]:
        ...
        if "native-access=true" not in measurements[REGULATOR_ENGINE]["engine_version"]:
            raise ValueError(f"Workload {name!r} primary Regulator runner did not report native access")
        if "native-access=false" not in measurements[REGULATOR_OBJECT_ENGINE]["engine_version"]:
            raise ValueError(f"Workload {name!r} object Regulator runner reported native access")
...
def normalize_rows(workloads, allow_native_drift=False, exclude_native_drift=False):
    ...
    for name, measurements in sorted(workloads.items()):
        ...
        if native_drift > DRIFT_LIMIT + 1e-12:
            drift_rows.append({
                "name": name,
                "model": measurements[REGULATOR_ENGINE]["model"],
        ...
        regulator_median = medians[REGULATOR_ENGINE]
        regulator_object_median = medians[REGULATOR_OBJECT_ENGINE]
        regulator_to_host_ratio = regulator_median / host_bracketed
        absolute_deficit_ns = regulator_median - host_bracketed
        material = (
            (regulator_to_host_ratio > 1.10 and absolute_deficit_ns > 1_000) or
            regulator_to_host_ratio > 2 or
            regulator_to_host_ratio < 0.5)
        bundled_median = medians.get(BUNDLED_ENGINE)
        rows.append({
            "name": name,
            "model": measurements[REGULATOR_ENGINE]["model"],
            "haystack_len": measurements[REGULATOR_ENGINE]["haystack_len"],
            "regulator_median_ns": regulator_median,
            "regulator_object_median_ns": regulator_object_median,
            "re2_bundled_median_ns": "" if bundled_median is None else bundled_median,
            "re2_pinned_portable_median_ns": medians[PINNED_PORTABLE_ENGINE],
            "re2_pinned_host_tuned_before_median_ns": host_before,
            "re2_pinned_host_tuned_after_median_ns": host_after,
            "re2_pinned_host_tuned_bracketed_median_ns": host_bracketed,
            "regulator_to_re2_bundled_ratio": "" if bundled_median is None else regulator_median / bundled_median,
            "regulator_to_re2_pinned_portable_ratio": regulator_median / medians[PINNED_PORTABLE_ENGINE],
            "regulator_to_re2_pinned_host_tuned_ratio": regulator_to_host_ratio,
            "regulator_object_to_re2_pinned_host_tuned_ratio": regulator_object_median / host_bracketed,
            "regulator_to_regulator_object_ratio": regulator_median / regulator_object_median,
            "native_before_after_drift": native_drift,
            "regulator_coefficient_of_variation": measurements[REGULATOR_ENGINE]["coefficient_of_variation"],
...
def reduce_results(
        ...
        exclude_bundled_engine=False):
    global REQUIRED_ENGINES
    global COMPARATORS
    REQUIRED_ENGINES = CORE_REQUIRED_ENGINES if exclude_bundled_engine else (BUNDLED_ENGINE, *CORE_REQUIRED_ENGINES)
    COMPARATORS = CORE_COMPARATORS if exclude_bundled_engine else ((BUNDLED_ENGINE, "regulator_to_re2_bundled_ratio"), *CORE_COMPARATORS)
...
def main():
    ...
    parser.add_argument("--exclude-bundled-engine", action="store_true")
    ...
    try:
        ...
        reduce_results(
            ...
            arguments.exclude_bundled_engine)
