...
abstract class RegexpWalker<T>
{
    ...
    final T walk(Regexp regexp, T initialValue)
    {
        return walkInternal(regexp, initialValue, true, false, DEFAULT_MAX_VISITS);
    }

    final T walkExponential(Regexp regexp, T initialValue, int maxVisits)
    {
        if (maxVisits <= 0) {
    ...
    private T walkInternal(Regexp regexp, T initialValue, boolean copyDuplicateSiblings, boolean allowVisitLimitFallback, int maxVisits)
    {
        ...
        while (!stack.isEmpty()) {
            ...
            if (!state.preVisitCompleted) {
                ...
                if (state.skipChildren || state.regexp.childCount() == 0) {
                ...
                state.childValues = new ArrayList<>(state.regexp.childCount());
                for (int childIndex = 0; childIndex < state.regexp.childCount(); childIndex++) {
                    state.childValues.add(null);
                }
            }

            int childIndex = state.nextChildIndex;
            if (childIndex >= state.regexp.childCount()) {
            ...
            Regexp child = state.regexp.child(childIndex);
            T childValue = state.preVisitValue;
            if (copyDuplicateSiblings && childIndex > 0 && child == state.regexp.child(childIndex - 1)) {
    ...
    record PreVisitResult<T>(T value, boolean stop) {}
