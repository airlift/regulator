...
public class TestRe2Match
{
    ...
    public void testMatchNumberPeculiarity()
    {
        ...
        assertThat(re2.find(utf8("f"))).isFalse();

        MatchResult alternation = Re2.compile(utf8("(foo)|hello"), Re2.Options.defaults()).matchesResult(utf8("hello"));
    ...
    public void testMatch()
    {
        ...
        MatchResult result = re2.findResult(utf8("a chrisr:9000 here"));
    ...
    public void testFoldCasePrefixAcceleration()
    {
        ...
        assertThat(re2.find(utf8("HELLO world"))).isTrue();
        assertThat(re2.find(utf8("hello world"))).isTrue();
        assertThat(re2.find(utf8("HeLLo world"))).isTrue();

        assertThat(re2.find(utf8("HELL"))).isFalse();
        assertThat(re2.find(utf8("xhello"))).isTrue();
    }
    ...
    public void testInlineFlagScope()
    {
        ...
        assertThat(scoped.matches(utf8("Ab"))).isTrue();
        assertThat(scoped.matches(utf8("AB"))).isFalse();

        Re2 nestedDirective = Re2.compile(utf8("((?i)a)b"), Re2.Options.defaults());
        assertThat(nestedDirective.matches(utf8("Ab"))).isTrue();
        assertThat(nestedDirective.matches(utf8("AB"))).isFalse();

        Re2 unscoped = Re2.compile(utf8("(?i)ab"), Re2.Options.defaults());
        assertThat(unscoped.matches(utf8("AB"))).isTrue();

        assertThat(Re2.compile(utf8("(?-i)a"), Re2.Options.defaults()).matches(utf8("A"))).isFalse();
        assertThat(Re2.compile(utf8("(?i-m:a)"), Re2.Options.defaults()).matches(utf8("A"))).isTrue();
        assertThat(Re2.compile(utf8("(?i-i)a"), Re2.Options.defaults()).matches(utf8("A"))).isFalse();
        assertThat(Re2.compile(utf8("(?)"), Re2.Options.defaults()).matches(utf8(""))).isTrue();
    }

    @Test
    public void testPosixWordClassIsAscii()
    {
        for (Re2.Options options : new Re2.Options[] {
                Re2.Options.defaults(),
                Re2.Options.latin1(),
        }) {
            Re2 word = Re2.compile(utf8("[[:word:]]+"), options);
            assertThat(word.matches(utf8("09AZ_az"))).isTrue();
            assertThat(word.find(utf8("-"))).isFalse();
            assertThat(word.find(utf8("é"))).isFalse();

            Re2 nonWord = Re2.compile(utf8("[[:^word:]]+"), options);
            assertThat(nonWord.matches(utf8("-"))).isTrue();
            assertThat(nonWord.find(utf8("A"))).isFalse();
        }
    }

    @Test
    public void testUnterminatedPosixClassNameFallsBackToLiterals()
    {
        Re2 regexp = Re2.compile(utf8("[[:foo1]"), Re2.Options.defaults());

        assertThat(regexp.matches(utf8("1"))).isTrue();
        assertThat(regexp.matches(utf8(":"))).isTrue();
        assertThat(regexp.matches(utf8("]"))).isFalse();
    }
    ...
    public void testPrefixOptimizationFastRejection()
    {
        ...
        assertThat(re2.find(utf8("foobar123"))).isTrue();
        assertThat(re2.find(utf8("xfoobar"))).isFalse();
        assertThat(re2.find(utf8("foo"))).isFalse();
    }
    ...
    public void testAnchorModeUpgrade()
    {
        ...
        assertThat(re2.matches(utf8("foo"))).isTrue();

        assertThat(re2.find(utf8("afoo"))).isFalse();
        assertThat(re2.find(utf8("foob"))).isFalse();

        Re2 startAnchored = Re2.compile(utf8("^foo"), Re2.Options.defaults());
        assertThat(startAnchored.find(utf8("foobar"))).isTrue();
        assertThat(startAnchored.find(utf8("xfoo"))).isFalse();
    }
}
