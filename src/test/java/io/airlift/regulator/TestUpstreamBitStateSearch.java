...
public class TestUpstreamBitStateSearch
{
    private static Prog compile(String pattern, int flags)
    {
        ...
        assertThat(prog.canBitState()).as("canBitState: %s", pattern).isTrue();
        return prog;
    }
    ...
    public void testLeftmostFirstVsLeftmostLongest()
    {
        ...
        int[] first = new int[4];
        assertThat(BitState.search(prog, text, true, Prog.MatchKind.FIRST_MATCH, first)).isTrue();
        ...
        assertThat(BitState.search(prog, text, true, Prog.MatchKind.LONGEST_MATCH, longest)).isTrue();
    ...
    public void testUnanchoredLeftmostFirstMatch()
    {
        ...
        int[] m = new int[2];
        assertThat(BitState.search(prog, text, false, Prog.MatchKind.FIRST_MATCH, m)).isTrue();
    ...
    public void testAnyByteStarGreedyVsNonGreedy()
    {
        ...
        int[] g = new int[2];
        assertThat(BitState.search(greedy, text, true, Prog.MatchKind.FIRST_MATCH, g)).isTrue();
        assertThat(g[0]).isEqualTo(0);
        assertThat(g[1]).isEqualTo(3);

        Prog nonGreedy = compile("\\C*?", Regexp.LIKE_PERL | Regexp.LATIN1);
        int[] ng = new int[2];
        assertThat(BitState.search(nonGreedy, text, true, Prog.MatchKind.FIRST_MATCH, ng)).isTrue();
