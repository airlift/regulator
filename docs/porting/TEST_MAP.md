...
This map connects Regulator's tests to pinned upstream RE2 coverage. A test
derived from an upstream file does not necessarily cover every case or
parameter in that file; the table records those distinctions.
...
- `src/test/resources/io/airlift/regulator/upstream_public_match.jsonl`
- `io.airlift.regulator.TestUpstreamPublicMatch`
...
- `src/test/resources/io/airlift/regulator/testing/native_longest_boundary.jsonl`
- `io.airlift.regulator.TestNativeLongestBoundaryAgreement`
...
|---|---|---|---|
| `string_generator_test.cc` | Ported corpus | `io.airlift.regulator.TestStringGenerator` | None identified. |
| `regexp_test.cc` | Ported corpus | `io.airlift.regulator.TestRegexpAst`, `TestRegexpWalker`, `TestRegexpDump` | Manual reference-count tests are not applicable to Java. |
| `parse_test.cc` | Ported corpus | `io.airlift.regulator.TestUpstreamParseDump`, `TestUpstreamBadRegexps`, `TestUpstreamParseErrorArguments`, `TestUpstreamParseEscape`, `TestUpstreamSyntaxModes`, `TestRegexpParser` | None identified. |
| `simplify_test.cc` | Ported corpus | `io.airlift.regulator.TestUpstreamSimplifyToString`, `TestSimplifier` | Verify future upstream additions when the pin changes. |
| `compile_test.cc` | Ported corpus | `io.airlift.regulator.TestUpstreamCompileDump`, `TestUpstreamCompileByteMap`, `TestUpstreamCompileMemoryBudget`, compiler bug regressions | None identified. |
| `possible_match_test.cc` | Ported corpus and parameters | `io.airlift.regulator.TestUpstreamPossibleMatchRange`, `TestUpstreamPossibleMatchRangeExhaustive` | None identified. |
| `search_test.cc` | Ported corpus and engine matrix | `io.airlift.regulator.TestUpstreamSearch`, direct-engine tests, `Tester` | All 236 rows run through five parse modes, first/longest/full matching, Backtrack, NFA, BitState, eligible OnePass, boolean DFA, and forward/reverse boundary DFA. |
| `dfa_test.cc` | Ported corpus and parameters | `io.airlift.regulator.TestUpstreamDfa`, `TestDfaConcurrency`, reverse/context/full-match tests | Exact build, search, concurrency, memory, and reverse parameters are retained. Seven callback rows assert the exact graph dump; three accepting-loop rows assert the upstream state count and longest boundary because Java represents the accepting loop with its full-match sentinel. Upstream malloc-counter assertions are disabled in the pinned source and are not applicable. |
| `exhaustive_test.cc` | Ported parameters | `io.airlift.regulator.TestExhaustiveAgreement` | All four active Egrep invocations are represented. |
| `exhaustive1_test.cc` | Ported parameters | `io.airlift.regulator.TestExhaustiveRepetition` | All four active repetition invocations are represented. |
| `exhaustive2_test.cc` | Ported parameters | `io.airlift.regulator.TestExhaustiveEdgeCase` | All three active invocations are represented. |
| `exhaustive3_test.cc` | Ported parameters | `io.airlift.regulator.TestExhaustiveCharClass` | Both character-class and both interesting-UTF-8 invocations are represented. |
| `random_test.cc` | Ported parameters with native oracle | `io.airlift.regulator.TestRandomAgreement` | Five groups preserve seed 404, 100 constructions, four wrappers, and 100 texts per construction. The 200,000 pinned native cases record exact match and capture results and run through the full Java engine/public matrix. Upstream's null-input probe is not applicable to the null-rejecting Slice API. |
| `charclass_test.cc` | Ported corpus | `io.airlift.regulator.TestUpstreamCharClassBuilder`, `TestCharClassBuilder`, exhaustive character-class tests | All 18 unique rows are covered; upstream contains one duplicate row. |
| `required_prefix_test.cc` | Ported corpus | `io.airlift.regulator.TestUpstreamRequiredPrefix`, `io.airlift.regulator.TestUpstreamPrefixAccel` | All three upstream test bodies and both case tables are represented. |
| `set_test.cc` | Ported corpus | `io.airlift.regulator.TestRe2Set` | All matching, anchoring, empty-set, prefix, builder, and memory-limit cases are covered. C++ move semantics are not applicable to Java. |
| `filtered_re2_test.cc` | Ported corpus | `io.airlift.regulator.TestFilteredRe2` | All atom, index, Latin-1, empty-pattern, builder, and matching cases are covered. C++ move semantics are not applicable to Java. |
...
`upstream_search_test_cases.base64` is the ordered pattern/text table from
`re2/testing/search_test.cc` at the pinned upstream commit, with each byte string
encoded independently as Base64. `TestUpstreamSearch` freezes the 236-row count
and the complete resource SHA-256. When the upstream pin changes, regenerate the
rows in source order, preserve C++ byte-string contents exactly, update the
count and checksum together, and review the fixture diff against the upstream
table.

Generated native-oracle fixtures are produced by the scripts under
`tools/re2-golden/`. Their generators verify the pinned source checkout before
writing results.
