...
public class BenchmarkRe2Parse
{
    ...
    public static class PhoneSearchState
    {
        ...
        Slice text;
        Re2 re2PhoneSearch;
        final int[] twoCaptureGroups = new int[6];
    ...
    @Benchmark
    public int[] searchPhoneRe2Captures(PhoneSearchState state)
    {
        if (!state.re2PhoneSearch.matchInto(state.text, Anchor.UNANCHORED, state.twoCaptureGroups)) {
            throw new IllegalStateException("Expected phone capture match");
        }
        return state.twoCaptureGroups;
    }
