    ...
    private static void dumpInto(StringBuilder output, Regexp regexp)
    {
        ...
        switch (regexp.op()) {
            ...
            case CONCAT, ALTERNATE -> {
                for (int index = 0; index < regexp.childCount(); index++) {
                    dumpInto(output, regexp.child(index));
                }
            }
            case STAR, PLUS, QUEST -> dumpInto(output, regexp.child(0));
            case CAPTURE -> {
                if (regexp.captureIndex() == 0) {
                ...
                dumpInto(output, regexp.child(0));
            }
            case REPEAT -> {
