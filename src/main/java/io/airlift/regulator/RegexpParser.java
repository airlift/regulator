...
final class RegexpParser
{
    ...
    private static final class Parser
    {
        ...
        private GroupFrame parseGroupStart()
        {
            ...
            if (!atEnd() && peekByte() == '?') {
                ...
                if (!atEnd()) {
                    int c = peekByte();
                    if (c == '=' || c == '!') {
                        throw error(RegexpParseErrorCode.BAD_PERL_OP, Slices.wrappedBuffer(bytes, groupStart, 3));
                    }
                    if (c == '<' && index + 1 < end) {
                        int c2 = bytes[index + 1] & 0xFF;
                        if (c2 == '=' || c2 == '!') {
                            throw error(RegexpParseErrorCode.BAD_PERL_OP, Slices.wrappedBuffer(bytes, groupStart, 4));
                        }
                    }
                }
                if (!atEnd() && peekByte() == ':') {
                    ...
                    consumeByte('P');
                    if (atEnd() || consumeByte() != '<') {
                        throw error(RegexpParseErrorCode.BAD_NAMED_CAPTURE, errorArgAtCurrent());
                    }
        ...
        private int parseInlineFlags(int base, int groupStart)
        {
            ...
            while (true) {
                if (atEnd()) {
                    throw error(RegexpParseErrorCode.BAD_PERL_OP, Slices.wrappedBuffer(bytes, groupStart, index - groupStart));
                }
                int c = peekByte();
                if (c == ')' || c == ':') {
                    if (negated && !sawFlags) {
                ...
                switch (rune) {
                    case 'i' -> inlineFlags = negated ? (inlineFlags & ~Regexp.FOLD_CASE) : (inlineFlags | Regexp.FOLD_CASE);
                    case 's' -> inlineFlags = negated ? (inlineFlags & ~Regexp.DOT_MATCHES_NEWLINE) : (inlineFlags | Regexp.DOT_MATCHES_NEWLINE);
                    case 'U' -> inlineFlags = negated ? (inlineFlags & ~Regexp.NON_GREEDY) : (inlineFlags | Regexp.NON_GREEDY);
                    // RE2 uses "OneLine" with inverted sense: -m means OneLine, +m means !OneLine.
                    case 'm' -> inlineFlags = negated ? (inlineFlags | Regexp.ONE_LINE) : (inlineFlags & ~Regexp.ONE_LINE);
        ...
        private Regexp parseCharClass()
        {
            ...
            while (!atEnd()) {
                ...
                if (nextByte == '-' && !first && (flags & Regexp.PERL_EXTENSIONS) == 0 && !isAtClassEndAfterDash()) {
                    throw error(RegexpParseErrorCode.BAD_CHAR_RANGE, errorArgDashWithFollowingRune());
                }
                first = false;
                ...
                if (nextByte == '\\' && index + 1 < end) {
                    ...
                    if (characterClass != null) {
                        if ((nodeFlags & Regexp.PERL_CLASSES) == 0) {
                            throw error(RegexpParseErrorCode.BAD_ESCAPE, null);
                        }
                        // consume '\' + escape char
                        index += 2;
        ...
        private boolean tryParsePosixCharClass(int parseFlags, CharClassBuilder characterClassBuilder)
        {
            ...
            int terminatorIndex = startIndex + 2;
            while (terminatorIndex + 1 < end && (bytes[terminatorIndex] != ':' || bytes[terminatorIndex + 1] != ']')) {
                terminatorIndex++;
            }
            if (terminatorIndex + 1 >= end) {
                return false;
            }

            int nameStart = startIndex + 2;
            boolean negate = nameStart < terminatorIndex && bytes[nameStart] == '^';
            if (negate) {
                nameStart++;
            }

            String name = new String(bytes, nameStart, terminatorIndex - nameStart, StandardCharsets.US_ASCII);
            CharClass characterClass = posixCharClass(name);
            if (characterClass == null) {
                throw error(
                        RegexpParseErrorCode.BAD_CHAR_RANGE,
                        Slices.wrappedBuffer(bytes, startIndex, (terminatorIndex + 2) - startIndex));
            }
