...
public final class RebarRunner
{
            ...
            throws Exception
    {
        if (arguments.length == 1 && arguments[0].equals("--version")) {
            System.out.printf(
                    "Regulator (%s %s, native-access=%s)%n",
        ...
        if (arguments.length != 0) {
            throw new IllegalArgumentException("usage: RebarRunner [--calibrate|--manifest|--version]");
        }
    ...
    private static long grepResult(Re2 pattern, Slice haystack, byte[] bytes, int byteArrayOffset)
    {
        ...
        while (lineStart < haystack.length()) {
            ...
            if (pattern.find(haystack.slice(lineStart, contentEnd - lineStart))) {
                count++;
            }
            lineStart = lineEnd + 1;
        }
    ...
    private static final class Samples
    {
        ...
        public void add(long duration, long count)
        {
            if (size == durations.length) {
                ...
                durations = Arrays.copyOf(durations, newSize);
